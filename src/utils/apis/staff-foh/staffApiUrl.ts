...
const prderComplete = `/${urlName}/foh/orderCompleted` // 外場完成訂單( 備餐完成 )
...
export const staffFetch = {
  ...
  orderData: (page = '', orderStatus = '4', type = '', orderBy = '', search = '') => api.get(orderData + `?page=${page}&orderStatus=${orderStatus}&type=${type}&orderBy=${orderBy}&search=${search}`),
  ...
  orderComplete: (data: any) => api.post(prderComplete, data)
  // ... 其他 API
}
